...
**HDR video** - High Dynamic Range video. A technology used to produce better looking video by preserving details in the darkest and lightest portions of a frame resulting in better contrast. For a HTPC, this means HDR support must be in the video, GPU, video interface (HDMI/DP) and ultimately your output device (typically a TV). Check the [FAQ](https://www.reddit.com/r/htpc/wiki/faq#wiki_what_is_hdr_video_and_what_do_i_need_to_take_advantage_of_it.3F) for GPUs that support HDR. Common standards are HDR10, HDR10+ and Dolby Vision.
