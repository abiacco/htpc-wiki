...
See also: [Video Software](/r/htpc/wiki/video#wiki_video_software)  
...
Consult [this section](/r/htpc/wiki/faq#wiki_video_cables.2Fadapters.3A) of the wiki FAQ for cable/adapter recommendations. Consult [this section](/r/htpc/wiki/audio#wiki_what_kind_of_sound_system_should_i_get.3F) below for sound system recommendations.  
...
You will use an HDMI splitter to split/clone the source signal and send the same signal to both your display and sound system. Note that not all splitters are built the same and can vary dramatically in price, depending on feature set and reliability. See the [accessories](/r/htpc/wiki/audio#wiki_accessories) section.  

1. Connect the HTPC's primary HDMI output to a [HDMI Splitter](/r/htpc/wiki/audio#wiki_accessories).
...
This scenario would be used for non-encoded audio, like **[Gaming](/r/htpc/wiki/audio#wiki_how_do_i_configure_my_htpc_to_output_sound_for_games_to_my_sound_system.3F)**, streaming services that only support stereo, sound systems where encoded codecs are not supported and/or for analog audio interfaces.  
...
STREAMING SERVICES:  
1. Make sure your service supports a bitstreamable codec (Dolby/DD). See: [Wiki:FAQ:What resolution and audio is supported on streaming service X?](/r/htpc/wiki/faq#wiki_what_resolution_and_audio_is_supported_on_streaming_service_x.3F)   
...
4. Restart the app and play test content from the [sample audio](/r/htpc/wiki/audio#wiki_where_do_i_find_sample_audio_files_to_test.3F) section below.  
...
3. Restart the app and play test content from the [sample audio](/r/htpc/wiki/audio#wiki_where_do_i_find_sample_audio_files_to_test.3F) section below.  
...
2. Restart the app and play test content from the [sample audio](/r/htpc/wiki/audio#wiki_where_do_i_find_sample_audio_files_to_test.3F) section below.  
...
6. Restart the app and play test content from the [sample audio](/r/htpc/wiki/audio#wiki_where_do_i_find_sample_audio_files_to_test.3F) section below.  
...
4. Restart your media player app  and play test content from the [sample audio](/r/htpc/wiki/audio#wiki_where_do_i_find_sample_audio_files_to_test.3F) section below.  
5. If configured correctly, you should see the blue LAV Audio icon in the system tray. Click on it and then on the Status Tab to see the audio being bitstreamed.  
...
Streaming Services: See [Wiki:FAQ:What resolution and audio is supported on streaming service X?](/r/htpc/wiki/faq#wiki_what_resolution_and_audio_is_supported_on_streaming_service_x.3F)  

Gaming: [PCM 5.1/7.1, Dolby Digital Live w/supported sound card or APO, Dolby Atmos w/Dolby Access app](/r/htpc/wiki/audio#wiki_how_do_i_configure_my_htpc_to_output_sound_for_games_to_my_sound_system.3F)
...
3. You're trying to send/passthrough codecs over an unsupported audio interface or device.  
 ...
 If they don't, you'll have to change the way you connect things (see scenarios above), if possible OR have the unsupported codec transcoded into another, supported codec, either in your client or via a capable media server (like Plex). See [this section](/r/htpc/wiki/audio#wiki_my_sound_system_only_supports_decoding_dolby_digital.2C_but_i_want_to_play_content_with_varying_formats._how_to_i_re-encode_it_all_to_dolby_digital.3F) below.  
4. You're trying to pass game audio through an unsupported audio interface/device.  
 ...
 proper interfaces for PCM 5.1. If you don't have one, see [this section](/r/htpc/wiki/audio#wiki_how_do_i_configure_my_htpc_to_output_sound_for_games_to_my_sound_system.3F) below for how to convert Game audio into compatible Dolby Digital Live audio instead.  
5. The PC is detecting the number of supported channels and codecs from your sound system incorrectly  
 ...
 See the [next section](/r/htpc/wiki/audio#wiki_why_does_my_operating_system_only_show_stereo.2F2-channels_in_the_sound_control_panel_when_i_have_a_surround_sound_channel_system.3F) for how to fix this.  
...
SYSTEM-WIDE: See the [section below](/r/htpc/wiki/audio#wiki_how_do_i_configure_my_htpc_to_output_sound_for_games_to_my_sound_system.3F) on encoding gaming audio, which does the same thing, but on a lower-level.  
...
You can use a good HDMI 2.0 splitter to send 4k/60hz video to a display and HD audio (TrueHD/DTS:X/etc..) to an old AVR that only supports HDMI 1.3/1.4. Find an old/now-discontinued HD Fury AVR Key or Integral. If not, then the ViewHD or SIIG in the "[HDMI 2.0 Splitters](/r/htpc/wiki/audio#wiki_accessories)" section below are the best bets. They won't work with all hardware combinations, and you may have to fiddle with the EDID dip switches quite a bit in the initial setup, but should work.  
...
If you have a USB-C port (that supports DP Alt Mode video out) you can use a [USB-C to HDMI cable/adapter](/r/htpc/wiki/faq#wiki_video_cables.2Fadapters.3A) to do the same thing. Your mileage may vary depending on the adapter and source device, so use a recommended adapter/cable.  
...
See the [Video Cables/Adapters](/r/htpc/wiki/faq#wiki_video_cables.2Fadapters.3A) section of the Wiki FAQ for more recommendations.  
